...
function friendsOfRecursion(name, adjacencyList, visited, maxDistance, currentDistance) {
  ...
  visited.add(name);
  adjacencyList[name].forEach(neighbor => {
      friendsOfRecursion(neighbor, adjacencyList, visited);
  })
...
//   if (visited.has(name)) return false;
//   visited.add(name)
//   for (let i = currentDistance; i < maxDistance; i++) {
//     adjacencyList[name].forEach(neighbor => friendsOfRecursion(neighbor, adjacencyList, visited, maxDistance))
//   }
// return Array.from(visited)
}

function friendsOf(adjacencyList, name, distance) {
  if (!Object.keys(adjacencyList).length) return undefined;
  let visited = new Set();
  return friendsOfRecursion(name, adjacencyList, visited, distance, currentDistance = 0)
}
console.log(friendsOf(graph, "jun", 1))
